...
<body>
    ...
    if ($result->num_rows > 0) {
        ...
        foreach($pedidos as $pedidoID => $detalle) {
            ...
            echo '</ol>';
            echo '</div>';
            echo '<button onclick="aceptarPedido(' . $pedidoID . ')">Aceptar</button>';
            echo '<button onclick="rechazarPedido(' . $pedidoID . ')">Rechazar</button>';
            echo '</div>';
        }
    } else {
    ...
    <footer>
        <div class="final">
            <a href="../V_A_Menu/index.php" class="boton-volver">Volver al índice de la empresa</a>
        </div>
    </footer>
